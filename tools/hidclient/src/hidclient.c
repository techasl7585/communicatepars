...
char		rotate_mouse_cw  = 0;	// rotate relative mouse axes 90 degrees clockwise
			  ...
			  case	REL_WHEEL:
				...
				evmouse->axis_z =
					( inevent->code >= ABS_Z ?
					  inevent->value : 0 );
				/*
				 * iOS rotates legacy relative HID mouse axes with a
				 * portrait display. CommunicatePars enables this
				 * inverse transform only for its "portrait iPhone"
				 * mode. Landscape/tablet behavior stays byte-for-byte
				 * compatible with the original path above.
				 */
				if ( rotate_mouse_cw )
				{
					int rotated_value = inevent->value;
					if ( rotated_value > 127 ) rotated_value = 127;
					if ( rotated_value < -127 ) rotated_value = -127;
					evmouse->axis_x =
						( inevent->code == ABS_Y ?
						  -rotated_value : 0 );
...
int	main ( int argc, char ** argv )
{
	...
	for ( i = 1; i < argc; ++i )
	{
		...
		else if ( 0 == strcmp ( argv[i], "--rotate-cw" ) )
		{
			rotate_mouse_cw = 1;
		}
...
"--rotate-cw	Rotate relative mouse axes 90 degrees clockwise\n" \
